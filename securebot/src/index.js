...
app.get("/", (req, res) => {
  res.json({
    ...
    documentation: {
      github_app_setup:
        "Create a GitHub App and set GITHUB_APP_ID and GITHUB_PRIVATE_KEY",
      ai_setup: "Set GOOGLE_AI_API_KEY for automated fixing",
      usage:
        "Install the GitHub App on your repositories, then use the API endpoints",
    },
...
app.listen(port, () => {
  ...
  console.log(
    "   GET  /api/installation/status       - Check GitHub App installation"
  );
  console.log(
    "   GET  /api/user/<username>/repositories - Get user repositories"
  );
  console.log(
    "   POST /api/scan                      - Scan repository for security issues"
  );
  console.log(
    "   POST /api/fix                       - Fix issues and create PR"
  );
  console.log(
    "   GET  /api/repositories/cloned       - List cloned repositories"
  );
