...
const AuthorizeGitHubTab = () => {
  ...
  const [secureBotStatus, setSecureBotStatus] = useState(null);
  ...
  useEffect(() => {
    ...
    if (githubConnected === "true") {
      ...
      console.log("GitHub OAuth successful, URL cleaned up");
    } else if (error) {
      const errorMessages = {
      ...
      toast.error(
        errorMessages[error] || "GitHub connection failed. Please try again."
      );
      // Clean up URL
      const newUrl = window.location.pathname;
      window.history.replaceState({}, document.title, newUrl);
  ...
  // Fetch repositories when GitHub user is set and connected
  useEffect(() => {
  ...
  const checkGitHubConnection = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/user/github/status`,
      ...
      // Fetch repositories if user is connected
      if (response.data.githubUser) {
        fetchRepositoriesFromSecureBot();
      }
    } catch (error) {
      console.log(
        "GitHub not connected or error:",
  ...
  const handleGitHubAuth = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/user/github/auth`,
        { withCredentials: true }
      );
  ...
  const checkSecureBotStatus = async () => {
    try {
      ...
      // If SecureBot is installed and user is connected, fetch repositories
  ...
  const fetchRepositoriesFromSecureBot = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_SECUREBOT_URL}/user/${githubUser}/repositories`
      );
      ...
      if (response.data.success) {
        ...
        setSecureBotStatus(
          response.data.installed
            ? {
                ...
                scans: 0, // TODO: Get actual scan count
              }
            : null
        );
      } else {
        ...
        if (!response.data.installed) {
          toast.info("Install SecureBot GitHub App to see your repositories");
        }
      }
    } catch (error) {
      console.error("Error fetching repositories from SecureBot:", error);
      setRepositories([]);
      // Don't show error toast as this might be expected if SecureBot is not installed
    } finally {
  ...
  const handleDisconnect = async () => {
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/user/github/disconnect`,
        {},
        { withCredentials: true }
      );
      setIsConnected(false);
